...
\emph{signal}, jest odpowiednikiem \emph{Singal Processing Toolbox} w
...
do automatycznego tłumaczenia z języka MATLABa. Scilab posiada dobrą dokumentację.
...
przez \emph{Scilab Enterprises}, w celu wyświetlania formuła matematycznych
...
komentarze blokowe. Podobnie jak w GNU Octave, można kończyć konstrukcję
kontrolujące przepływ, takie jak \lstinline|for| pasującymi słowami
kluczowymi (\lstinline|endfor| w tym przypadku). Tekst zawarty pomiędzy
...
\begin{table}[!htb]
  \caption{Funkcja licząca moduł liczby rzeczywistej w języku Python}
  \begin{lstlisting}
def modul(x):
    if x < 0:
        return -x
