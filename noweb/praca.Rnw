...
Algorytm stosowany badane pakiety, za wyjątkiem
...
nieefektywna we wczytywaniu plików dłuższych niż ok \SI{0.2}{\second},
...
<<summary.intel, fig.cap="Wyniki testów dla najdłuższego wektora (Laptop)">>=
summaryPlot(results.intel)

@ 

<<summary.amd, fig.cap="Wyniki testów dla najdłuższego wektora (PC)">>=
