...
krótszym na komputerze stacjonarnym. Może to wskazywać
na wysoką wielowątkowość algorytmu z którego
korzysta \emph{NumPy}, lub też na wykorzystanie Intel\textregistered\
Math Kernel Library (patrz punkt \ref{sec:anaconda}) przez tę funkcję.
...
można łatwo spostrzec, że nie ma jednego pakietu, którego
zastosowanie w każdej sytuacji pozwoli uzyskać wyniki w
najkrótszym czasie. W każdym z wykonanych testów inne
pakiety były w stanie wykonać testowaną funkcję najszybciej.
...
powody by nie korzystać z tego pakietu, lecz wyszukać
skorzystać z alternatywy.
