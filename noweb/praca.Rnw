...
jako cel kompilacji \emph{JIT} wystarczy przed jej definicją umieścić marker
\lstinline{@jit}.

Praca w Anacondzie jest możliwa przy pomocy trzech interfejsów. Można pracować
wyłącznie w wierszu poleceń, za pomocą aplikacji \emph{ipython}[odnośnik]
(\emph{interactive python}).
