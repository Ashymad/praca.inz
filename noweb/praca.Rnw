...
results.file <- h5file("/mnt/data/Documents/Workspace/Thesis/tests/results.h5")
results.list <- getResults(results.file)
@ 
...
Linuxa to 4.14.8\_rt9. Wykorzystane wersje pakietów to:
\begin{itemize}%
...
Skrypt uruchamia każdy z pakietów za pomocą komendy \emph{chrt},
...
wykonywania się skryptu za pierwszym razem. Po wykonaniu
odpowiedniej ilości testów (ilość ustalana jest w pliku
\emph{test.sh} i następnie przekazywana do pliku \emph{options.\$2}
jako zmienna \emph{number\_of\_tests})
zapisuje wyniki do pliku w formacie HDF5.
...
były najlepsze -- \emph{scipy.fftpack.fft} z biblioteki \emph{SciPy}.
\begin{table}[h]%
...
dokładnie tę samą funkcję. Funkcją tą jest algorytm FFT,
opisany przez <Imię Autora>[Odnośnik], zaadaptowany do
...
ten algorytm operuje tylko na tablicach, których rozmiar
...
\begin{table}[h]%
  ...
  \lstinputlisting[language=Python, firstline=2]{../tests/python/tests/conv/test.py}%
  \label{tab:convtest}%
\end{table}%
...
spowolnieniem zasobów przez nie wykorzystywanych.
...
preparePlot(results.list$four1)
...
preparePlot(results.list$wavread)
