...
podobieństwo do MATLABa, oraz popularność\cite{TIOBE,PyPl}.  

\section{MATLAB}
\label{sec:matlab}
\lstset{language=MATLAB}

MATLAB\cite{MATLAB} to środowisko do obliczeń numerycznych w szczególności
...
wykonywane są instrukcje po \lstinline|else| (Kod żr. \ref{tab:abs.m}).
...
GNU Octave\cite{Octave} to darmowy, otwartoźródłowy (pod licencją GPL)
pakiet obliczeniowy do
...
Scilab\cite{Scilab} jest darmowym, otwartoźródłowym (pod licencją GPL od wersji 6.0) pakietem
...
\emph{NumPy}\cite{NumPy} jest to biblioteka, która dodaje wielowymiarowe typy wektorowe
...
\emph{mplot3d} rozszerza funkcjonalność biblioteki o możliwość
rysowania wykresów trójwymiarowych.

Biblioteki \emph{NumExpr}\cite{NumExpr} oraz \emph{NumBa}\cite{NumBa} obie służą do zwiększenia
...
obiektów. Drugim interfejsem jest zintegrowane środowisko programistyczne \emph{Spyder}\cite{Spyder},
...
JuliaPro\cite{JuliaPro} jest to dystrybucja języka programowania \emph{Julia}\cite{Julia}
...
na GitHubie\cite{METADATA}. Aby zainstalować bibliotekę należy wykorzystać funkcję
...
podobne jak biblioteka GNU Readline (patrz punkt \ref{sec:octave}). Juno\cite{Juno}
...
znacznie przyspieszając obliczenia.
...
funkcja \lstinline{four1()} nie chociaż ani trochę tak
...
\begin{appendices}
  ...
  \chapter{Kod funkcji \lstinline|four1()| (Julia)}
  \label{chap:zalfour1}
