...
getResults <- function(file) {
    ...
    tests <- c("sin", "conv", "fft", "four1", "wavread", "pass")
    results <- list()
    for (test_name in tests) {
        ...
        for (package in packages) {
            ...
            for (input_size in 1:max_input_size) {
                res <- as.vector(file[test_name][package][toString(input_size)][])
...
priorytecie[BIBTEX:https://lwn.net/Articles/146861/]. Dzięki temu można mieć pewność, że
żaden proces uruchomiony w tle nie spowoduje zaniżonych wyników
badań. 

Wykorzystano dystrybucję \emph{Manjaro Linux}, gdyż posiada ona
RTLinux w oficjalnych repozytoriach. Wykorzystana wersja
Linuxa to 4.11.12\_rt16. Wykorzystane wersje pakietów to:
\begin{itemize}%
...
Najważniejszą częścią skryptu testującego jest funkcja \emph{testdir}
...
\end{figure}%
\begin{table}[h]%
  \caption{Kod pliku testującego (Julia)}%
  \lstinputlisting[language=Julia]{../tests/julia/tester.jl}%
  \label{tab:tester}%
\end{table}%
Funkcja \emph{testdir} uruchamiana jest dla każdego z pakietów
...
Argumenty pierwszy i drugi przekazywane są do funkcji \emph{testdir} jako
odpowiednio \emph{\$3} i \emph{\$TESTARG}, natomiast zmienna
\emph{\$INPUT\_SIZES} zawiera szereg kolejnych potęg liczby dziesięć,
aż do maksymalnego rzędu wielkości ilości elementów wektora wejściowego.
Jak można zauważyć wyniki zapisywanie są w pliku w formacie HDF5 --
...
biblioteka) i zawiera informację o strukturze, przypominającej
foldery i pliki.
...
znajdywać się funkcje: \emph{prepare\_input} -- 
przygotowuje dane wejściowe dla testowanej funkcji, jako
argument przyjmuje ilość elementów wektora danych,
\emph{test\_function} -- testowana funkcja, przyjmuje
dane przygotowane przez funkcję \emph{prepare\_input},
przetwarza je i zwraca wynik. W kolejnych punktach zostaną
opisane pokrótce te algorytmy wraz z przykładowym kodem
w jednym z testowanych języków.
...
funkcję \emph{sin()} na tych danych (Rys. \ref{tab:sintest}.

\begin{table}[h]%
  \caption{Kod testu \emph{sin} (MATLAB/GNU Octave)}%
  \lstinputlisting[language=MATLAB, firstline=6]{../tests/matlab/tests/sin/test.m}%
  \label{tab:sintest}%
\end{table}%
...
Funkcja testowana wywołuje na tym sygnale funkcję \emph{fft()}
(Tab. \ref{tab:ffttest}). Kod testu oparty jest na dokumentacji
funkcji \emph{fft()} dla pakietu MATLAB[odnośnik]. W pakiecie \emph{Anaconda} jest
dostępnych kilka implementacji FFT, wykorzystano tę której wyniki
były najlepsze -- \emph{scipy.fftpack.fft} z pakietu \emph{SciPy}.
\begin{table}[h]%
  \caption{Kod testu \emph{fft} (MATLAB/GNU Octave)}%
  \lstinputlisting[language=Octave, firstline=6]{../tests/octave/tests/fft/test.m}%
  \label{tab:ffttest}%
\end{table}%
...
\begin{table}[h]%
  \caption{Kod testu \emph{four1} (Julia)}%
  \lstinputlisting[language=julia, firstline=13]{../tests/julia/tests/four1/test.jl}%
  \label{tab:four1test}%
\end{table}%
...
\begin{table}[h]%
  \caption{Kod testu \emph{conv} (Python)}%
  \lstinputlisting[language=Python]{../tests/python/tests/conv/test.py}%
  \label{tab:convtest}%
\end{table}%
...
\begin{table}[h]%
  ...
  \lstinputlisting[language=Scilab]{../tests/scilab/tests/wavread/test.sci}%
