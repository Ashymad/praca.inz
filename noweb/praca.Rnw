...
GNU Octave\cite{OctMan} to darmowy, otwartoźródłowy (pod licencją GPL) pakiet obliczeniowy do
...
\emph{Anaconda Distribution}[odnośnik] to najpopularniejsza dystrybucja języka
programowania \emph{Python}\cite{PyRos} do zastosowań techniczno-naukowych,
...
JuliaPro[odnośnik] jest to dystrybucja języka programowania \emph{Julia}\cite{JulKarp}
