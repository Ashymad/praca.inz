...
dla AMD). Ta duża różnica może być spowodowana
wyjątkowo dobrą optymalizacją pętli przez Julię
(jak pokazano w punkcie \ref{sec:wynfftwla}).
...
Algorytm stosowany przez te pakiety opiera się
najprawdopodobniej na definicji splotu, nie jest to jednak
