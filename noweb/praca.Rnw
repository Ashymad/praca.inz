...
Poniżej znajduje się jeden z plików \emph{tester}. Jak można
zauważyć wyniki zapisywanie są w pliku w formacie HDF5 --
...
funkcji matematycznych, jaką jest sinus. Kod testu w
MATLABie/GNU Octave można znaleźć poniżej. 
\begin{table}[h]%
\caption{Kod testu \emph{sin} (MATLAB/GNU Octave)}%
\lstinputlisting[language=MATLAB, firstline=3]{../tests/matlab/tests/sin/test.m}%
\label{tab:sintest}%
\end{table}%

Jest to najprostszy test, funkcja przygotowująca dane
tworzy tablicę punktów rozmieszczonych w równych odległościach
pomiędzy $0$, a $2\pi$, natomiast funkcja testowana wywołuje
funkcję \emph{sin()} na tych danych.
...
\lstinputlisting[language=Octave]{../tests/octave/tests/fft/test.m}%
\label{tab:ffttest}%
\end{table}%
