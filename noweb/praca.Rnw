...
summaryPlot <- function(res) {
    ...
    plot <- plot + geom_point() + theme_bw() + scale_y_log10()
...
\@ifclassloaded{aghdpl}{
  \address{Armii Ludowej 49, 05-220 Zielonka}
  ...
  \specialisationEN{Vibrations and noise in engineering and the environment}
  ...
  \acknowledgements{%
    Serdecznie dziękuję dr. inż. Bartłomiejowi Borkowskiemu za nadzór
    merytoryczny i~Richardowi Stallmanowi za GNU.
  }
...
w funkcjonalność oraz najważniejsze różnice w działaniu
...
wersje badanych pakietów oraz narzędzia wykorzystane do
...
możliwych do podjęcia kroków w celu poprawienia
...
\label{tab:testdir}%
\end{program}%
Najważniejszą częścią skryptu testującego (Zał. \ref{chap:zaltestsh})
jest funkcja \lstinline{testdir()}
...
sygnałów audio, pozwalającym na wykonanie dyskretnej transformaty
...
z nim ich odpowiedzi impulsowych. Funkcja testowana
...
\begin{appendices}
  ...
  \chapter{Kod pliku \emph{test.sh} (Bash)}
  \label{chap:zaltestsh}
  {\singlespacing\lstinputlisting[language=Bash]{../tests/test.sh}}
\end{appendices}
