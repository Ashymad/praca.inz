...
przyjmowane argumenty oraz przekazuje opcje do pakietów, oraz
ze skryptów w każdym z testowanych języków, które uruchamiają
testy, mierzą czas wykonania, po czym zapisują wyniki do pliku.
Najważniejszą częścią skryptu testującego jest funkcja testdir,
...
pliku zostanie omówiona później.
...
\lstinputlisting[language=julia]{../tests/julia/tests/four1/test.jl}%

\subsection{Sinus}
\label{sec:sin}

\lstinputlisting[language=MATLAB]{../tests/matlab/tests/sin/test.m}%
